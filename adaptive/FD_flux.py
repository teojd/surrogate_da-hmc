...
#            Biot = np.minimum(Biot,200*np.ones_like(Biot))
    ...
    def solve(self, coefs=0):
        ...
        for j in range(self.n_t):
            b = tridiag_mult(B[j],Ui.reshape([-1,1])).flatten()
#            b = np.matmul(B[i],Ui.reshape([-1,1])).flatten()
            b = b - Dt*((1-thet)*Biot[j+1] + thet*Biot[j])
            b[0]= LBC[j];
            b[-1] = RBC[j];
            Ui = sp.linalg.solve_banded((1,1),A[j],b).reshape([-1,1]);
#            Ui = self.thomas();
            U = np.hstack([U,Ui]);
