...
class plot_estimates(object):
    ...
    def plot_map_cross(self,res_t=500,res_x=200,uncertainty=True):
        ...
        for j in range(6):
            ...
            funci1 = post_Bi_mean_1dx + 1.96*post_Bi_sd
            funci0 = post_Bi_mean_1dx - 1.96*post_Bi_sd
        ...
        if self.kind=='exp biot' or self.kind=='direct biot':        
            ...
            ''' for j in range(6):
                ...
                ma = np.max((map_Bi + 1.96*post_Bi_sd)*NN_solution)
                mi = np.min((map_Bi - 1.96*post_Bi_sd)*NN_solution)
    #                if self.positive==True:
    #                    post_Bi_samples_1dx = np.maximum(post_Bi_samples_1dx, np.zeros_like(post_Bi_samples_1dx))
                #post_Bi_samples_1dx = np.minimum(post_Bi_samples_1dx, 200*np.ones_like(post_Bi_samples_1dx))
            #    post_Bi_median_1dx = np.mean(post_Bi_samples_1dx,axis = 0)
            #    post_Bi_var = np.sqrt(np.var(post_Bi_samples_1dx,axis=0))
                funci1 = post_Bi_median_1dx + 1.96*post_Bi_var*sol_1dx.flatten()#np.percentile(post_Bi_samples_1dx,97.5,axis=0)# post_Bi_mean_1dx + post_Bi_var
                funci0 = post_Bi_median_1dx - 1.96*post_Bi_var*sol_1dx.flatten()# post_Bi_mean_1dx - post_Bi_var
                if self.csv_name == None:
            ...
            for j in range(len(plti)):
                ...
                ma = np.max((map_Bi + 1.96*post_Bi_sd))
                mi = np.min((map_Bi - 1.96*post_Bi_sd))
                funci1 = post_Bi_median_1dx + 1.96*post_Bi_var*sol_1dx.flatten()#np.percentile(post_Bi_samples_1dx,97.5,axis=0)# post_Bi_mean_1dx + post_Bi_var
                funci0 = post_Bi_median_1dx - 1.96*post_Bi_var*sol_1dx.flatten()# post_Bi_mean_1dx - post_Bi_var
                if self.csv_name == None:
                    biot_1d = self.true_biot[round(t_cross[j]/(self.tlim[1]-self.tlim[0])*(self.true_biot.shape[0]-1))]
            ...
            for j in range(len(plti)):
                ...
                ma = np.max(map_Bi + 1.96*post_Bi_sd)
                mi = np.min(map_Bi - 1.96*post_Bi_sd)
                funci1 = post_Bi_median_1dx + 1.96*post_Bi_var#np.percentile(post_Bi_samples_1dx,97.5,axis=0)# post_Bi_mean_1dx + post_Bi_var
                funci0 = post_Bi_median_1dx - 1.96*post_Bi_var# post_Bi_mean_1dx - post_Bi_var
                if self.csv_name == None:
                    biot_1d = self.true_biot[round(t_cross[j]/(self.tlim[1]-self.tlim[0])*(self.true_biot.shape[0]-1))]
                axs[plti[j]].plot(xx_1d,post_Bi_median_1dx)
    ...
    def plot_mcmc_function(self,res_t=2000,res_x=1000,burnin_prop = 0.8,n_thinned=500):
        ...
        if self.kind=='exp biot' or self.kind=='direct biot':        
            for j in range(len(plti)):
                ...
                post_Bi_median_1dx = np.mean(post_Bi_samples_1dx,axis = 0)
#                post_Bi_var = np.sqrt(np.var(post_Bi_samples_1dx,axis=0))
                ma = self.flux_scale*np.max(post_Bi_mean + 1.96*post_Bi_var)
                mi = self.flux_scale*np.min(post_Bi_mean - 1.96*post_Bi_var) 
...
#                post_Bi_var = np.sqrt(np.var(post_Bi_samples_1dx,axis=0))
            ...
            for j in range(len(plti)):
                ...
                post_Bi_median_1dx = np.mean(post_Bi_samples_1dx,axis = 0)
#                post_Bi_var = np.sqrt(np.var(post_Bi_samples_1dx,axis=0))
                ma = self.flux_scale*np.max(post_Bi_mean*NN_solution + 1.96*post_Bi_var)
                mi = self.flux_scale*np.min(post_Bi_mean*NN_solution - 1.96*post_Bi_var) 
                funci1 = np.percentile(post_Bi_samples_1dx,97.5,axis=0)# post_Bi_mean_1dx + post_Bi_var
                funci0 = np.percentile(post_Bi_samples_1dx,2.5,axis=0)# post_Bi_mean_1dx - post_Bi_var
                if self.csv_name == None:
...
#        post_Bi_samples = np.minimum(post_Bi_samples, 200*np.ones_like(post_Bi_samples))
        ...
        im = axs[1].pcolormesh(tt, xx, self.data.dat_scale*FD_solution.T, cmap=cm.coolwarm, vmin=mi, vmax=ma)
        fig.colorbar(im, ax=[axs[0],axs[1],axs[2]])
...
#        plt.show()
        ...
        for j in range(6):
            ...
            sol_1dx = FD_solution[:,int(np.round(tdat_1dx[0]*res_t/self.tlim[1]))-1]*self.data.dat_scale
            axs[plti[j],pltj[j]].plot(xx_1d,sol_1dx)
            axs[plti[j],pltj[j]].scatter(xdat_1dx,zdat_1dx,label=r"True $Bi(x)$", color='red',alpha = 0.4)
            axs[plti[j],pltj[j]].title.set_text('PDE solution at t =' + str(int(t_loc)))
            axs[plti[j],pltj[j]].set_ylim(mis-5,mas+5)
...
#        post_Bi_samples = np.minimum(post_Bi_samples, 200*np.ones_like(post_Bi_samples))
        ...
        FD_solution = FD_solver.solve(coefs_est)
#        FD_sol_nnres = griddata(np.hstack([xx_nn.reshape([-1,1]),tt_nn.reshape([-1,1])]), FD_solution.T.reshape([-1,1]), (xx, tt), method='linear').reshape([res_t,res_x])
        FD_sol_nnres = sp.interpolate.RectBivariateSpline(FD_solver.tmesh,FD_solver.xmesh, FD_solution.T).ev(tt_nn.reshape([-1,1]),xx_nn.reshape([-1,1])).reshape(xx_nn.shape)
        ...
        if self.kind=='exp biot' or self.kind=='direct biot':
            if self.csv_name == None:
                mi = self.flux_scale*np.min([np.min(post_Bi_mean*FD_sol_nnres),np.min(self.true_biot*self.true_sol)])
                ma = self.flux_scale*np.max([np.max(post_Bi_mean*FD_sol_nnres),np.max(self.true_biot*self.true_sol)])
            else:
                mi = self.flux_scale*np.min(post_Bi_mean*FD_sol_nnres)
                ma = self.flux_scale*np.max(post_Bi_mean*FD_sol_nnres)                    
        else:
            if self.csv_name == None:
                mi = self.flux_scale*np.min([np.min(post_Bi_mean),np.min(self.true_biot)])
                ma = self.flux_scale*np.max([np.max(post_Bi_mean),np.max(self.true_biot)])
        ...
        if self.csv_name == None:                
            ...
            if self.kind=='exp biot' or self.kind=='direct biot':
                miv = np.min(self.flux_scale*post_Bi_var*FD_sol_nnres)
                mav = np.max(self.flux_scale*post_Bi_var*FD_sol_nnres)
                im = axs[1].pcolormesh(tt_nn, xx_nn, self.flux_scale*post_Bi_mean*FD_sol_nnres, cmap=cm.coolwarm, vmin=mi, vmax=ma)
                fig.colorbar(im, ax=[axs[0],axs[1]])
                im = axs[2].pcolormesh(tt_nn, xx_nn, self.flux_scale*post_Bi_var*FD_sol_nnres, cmap=cm.coolwarm, vmin=0, vmax=mav)
                fig.colorbar(im, ax=axs[2])
            else:
                miv = np.min(self.flux_scale*post_Bi_var)
                mav = np.max(self.flux_scale*post_Bi_var)
            ...
            if self.kind=='exp biot' or self.kind=='direct biot':
                miv = np.min(self.flux_scale*post_Bi_var*FD_sol_nnres)
                mav = np.max(self.flux_scale*post_Bi_var*FD_sol_nnres)
                im = axs[0].pcolormesh(tt_nn, xx_nn, self.flux_scale*post_Bi_mean*FD_sol_nnres, cmap=cm.coolwarm, vmin=mi, vmax=ma)
                fig.colorbar(im, ax=[axs[0],axs[1]])
                im = axs[1].pcolormesh(tt_nn, xx_nn, self.flux_scale*post_Bi_var*FD_sol_nnres, cmap=cm.coolwarm, vmin=0, vmax=mav)
                fig.colorbar(im, ax=axs[1])
            else:
                miv = np.min(self.flux_scale*post_Bi_var)
                mav = np.max(self.flux_scale*post_Bi_var)
        ...
        if self.kind=='exp biot' or self.kind=='direct biot':
            for j in range(6):
                sol_1dx = FD_solution[:,int(np.round(t_cross[0]*res_t/self.tlim[1]))-1]*self.data.dat_scale
                post_Bi_samples_1dx = self.flux_scale*sol_1dx*sum(coefs_post[:,i].reshape([-1,1])*np.reshape(self.chebx.P(xx_1d,self.indices_x[i],tensor=False)*self.chebt.P(t_cross[j],self.indices_t[i],tensor=False),[1,nx_1d]) for i in range(self.indices_t.shape[0]))
                if self.positive==True:
                    post_Bi_samples_1dx = np.maximum(post_Bi_samples_1dx, np.zeros_like(post_Bi_samples_1dx))
                #post_Bi_samples_1dx = np.minimum(post_Bi_samples_1dx, 200*np.ones_like(post_Bi_samples_1dx))
                post_Bi_median_1dx = np.mean(post_Bi_samples_1dx,axis = 0)
#                post_Bi_var = np.sqrt(np.var(post_Bi_samples_1dx,axis=0))
                ma = self.flux_scale*np.max(post_Bi_mean*FD_sol_nnres + 1.96*post_Bi_var)
                mi = self.flux_scale*np.min(post_Bi_mean*FD_sol_nnres - 1.96*post_Bi_var) 
                funci1 = np.percentile(post_Bi_samples_1dx,97.5,axis=0)# post_Bi_mean_1dx + post_Bi_var
                funci0 = np.percentile(post_Bi_samples_1dx,2.5,axis=0)# post_Bi_mean_1dx - post_Bi_var
                if self.csv_name == None:
                    biot_1d = self.true_biot[round(t_cross[j]/(self.tlim[1]-self.tlim[0])*(self.true_biot.shape[0]-1))]*self.true_sol[round(t_cross[j]/(self.tlim[1]-self.tlim[0])*(self.true_biot.shape[0]-1))]
...
#        fig, axs = plt.subplots(2,3)
        ...
        for j in range(6):
            ...
            funci1 = post_Bi_mean_1dt + 1.96*post_Bi_sd
            funci0 = post_Bi_mean_1dt - 1.96*post_Bi_sd
...
#        FD_solution.T = griddata(np.hstack([xx_nn.reshape([-1,1]),tt_nn.reshape([-1,1])]), FD_solution.T.reshape([-1,1]), (xx, tt), method='linear').reshape([res_t,res_x])
#        FD_solution = sp.interpolate.RectBivariateSpline(FD_solver.tmesh,FD_solver.xmesh, FD_solution.T).ev(xx_nn.reshape([-1,1]),tt_nn.reshape([-1,1])).reshape(xx_nn.shape)
        ...
        im = axs[2].scatter(self.t_dat,resid,alpha=0.5,s=15)
#        im = axs[2].pcolormesh(tt, xx, self.data.dat_scale*FD_solution[:,:round(res_t*time_cutoff)].T, cmap=cm.coolwarm, vmin=mi, vmax=ma)
#        im = axs[2].pcolormesh(tt, xx, FD_solution[:,:round(res_t*time_cutoff)].T - FD_solution.T, cmap=cm.coolwarm, vmin=-0.005, vmax=0.005)
#        fig.colorbar(im, ax=[axs[2]])
        
        plt.show()
        
#        mi2 = np.min([np.min(FD_solution-True_solution),np.min(FD_solution.T.T-True_solution[:,:round(res_t*time_cutoff)])])
#        ma2 = np.max([np.max(FD_solution-True_solution),np.max(FD_solution.T.T-True_solution[:,:round(res_t*time_cutoff)])])
        
#        fig, axs = plt.subplots(2, 1)
        
#        axs[0].pcolormesh(tt_plt, xx_plt, FD_solution[:,:round(nt_FD*time_cutoff)].T - U[:,:round(nt_FD*time_cutoff)].T, cmap=cm.coolwarm, vmin=mi2, vmax=ma2)
#        im = axs[1].pcolormesh(tt_plt, xx_plt, FD_solution.T - U[:,:round(nt_FD*time_cutoff)].T, cmap=cm.coolwarm, vmin=mi2, vmax=ma2)
        
#        fig.colorbar(im, ax=axs.ravel().tolist())
#        plt.show()
        
        mi3 = np.min(FD_solution.T - FD_solution.T)
        ma3 = np.max(FD_solution.T - FD_solution.T)
        
        fig,ax = plt.subplots(1)
        im = plt.pcolormesh(tt, xx, FD_solution.T - FD_solution.T, cmap=cm.coolwarm, vmin=mi3, vmax=ma3)
        fig.colorbar(im)
        ...
        mas = np.max(FD_solution.T)*self.data.dat_scale
        mis = np.min(FD_solution.T)*self.data.dat_scale
